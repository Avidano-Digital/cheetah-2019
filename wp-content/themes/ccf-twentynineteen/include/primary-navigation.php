<ul class="extensible-list horizontal">
    <li>
        <a href="#1" title="About Us">About Us</a>
        
        <ul class="extensible-list sub">
            <li>
                <div class="row no-gutters" style="width:480px;">

                    <div class="col-6">
 
                        <ul class="extensible-list">
                            <li class="leader">What We Do</li>
                            <li><a href="/about-us/what-we-do/research/">Research</a></li>
                            <li><a href="/about-us/what-we-do/education/">Education</a></li>
                            <li><a href="/about-us/what-we-do/conservation/">Conservation</a></li>
                            <li><a href="/about-us/what-we-do/international-cheetah-day/">International Cheetah Day</a></li>
                            <li><a href="/about-us/what-we-do/article-tearsheet/">Article Tearsheet</a></li>
                        </ul>

                    </div>
                    <!-- .col -->

                    <div class="col-6">

                        <ul class="extensible-list">
                            <li class="leader">Who We Are</li>
                            <li><a href="/about-us/who-we-are/mission-and-vision/">Mission and Vision</a></li>
                            <li><a href="/about-us/who-we-are/dr-laurie-marker/">Dr. Laurie Marker</a></li>
                            <li><a href="/about-us/who-we-are/our-centre/">Our Centre</a></li>
                            <li><a href="#1">Staff and Board</a></li>
                            <li><a href="/about-us/who-we-are/ccf-global/">CCF Global</a></li>
                        </ul>

                    </div>
                    <!-- .col -->

                </div>
                <!-- .row -->

            </li>
        </ul>
    </li>
    <li>
        <a href="#1" title="Get Involved">Get Involved</a>
        <ul class="extensible-list sub">
            <li><a href="#1">Ways to Give</a></li>
            <li><a href="#1">Events</a></li>
            <li><a href="#1">Volunteer Locally</a></li>
            <li><a href="#1">Volunteer in Namibia</a></li>
            <li><a href="#1">Visit CCF</a></li>
            <li><a href="#1">Share</a></li>
        </ul>
    </li>
    <li>
        <a href="#1" title="Learn ">Learn</a>
        <ul class="extensible-list sub">
            <li><a href="/about-us/learn/resource-library/">Resource Library</a></li>
            <li><a href="#1">About Cheetahs</a></li>
            <li><a href="#1">Human Wildlife Conflict</a></li>
            <li><a href="#1">Illegal Pet Trade</a></li>
            <li><a href="#1">Habitat Loss</a></li>
            <li><a href="#1">CCF Videos</a></li>
        </ul>
    </li>
    <li> 
        <a <?php if( $inside_news !== false ) : ?> class="active" <?php endif; ?> href="#1" title="News">News</a>
        <ul class="extensible-list sub">
            <li><a href="/ccf-blog">CCF Blog</a></li>
            <li><a href="#1">Press Releases</a></li>
        </ul>
    </li>
    <li>
        <a class="btn btn-primary donate" href="#1" title="Donate">Donate</a>
    </li>
</ul>